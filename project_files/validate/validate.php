...
namespace Register {
    class Validate {
        ...
        public function birthday($name, $value)
        {
            $field = $this->fields->getField($name);
            $birthPattern = '/^(0[1-9]|1[012])\/(0[1-9]|1[0-9]|2[0-9]|3[0-2])\/[1-9][[:digit:]]{3}?/';
            $match = preg_match($birthPattern, $value);
            if( $match  == false)
            {
                $field->setErrorMessage('Error testing field.');
            }
            ...
            $dateDifference = $currDate->diff($inputDate);
            $match = preg_match('/-/',$dateDifference);
            if($match == 1)
            {
                $field->setErrorMessage('The date must be less than today.');
            }
            $field->clearErrorMessage();
        }
    }
